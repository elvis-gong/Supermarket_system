...
//  QSqlDatabase sqldb = QSqlDatabase::addDatabase("QMYSQL");
     // 创建一个数据库连接
     QSqlDatabase sqldb = QSqlDatabase::addDatabase("QMYSQL");
     sqldb.setHostName("localhost"); // 设置主机名
         sqldb.setDatabaseName("market"); // 设置数据库名称
     sqldb.setUserName("root"); // 设置用户名
     sqldb.setPassword("gfysql"); // 设置密码
  ...
  try
  {
      if (sqldb.open())
          ...
          throw QSqlError(sqldb.lastError());
  }
